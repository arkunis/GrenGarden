...
$cat_null = $db->getCatNull(["id_cat" => $_GET['categorie']]);
// print_r($cat_null);
?>
<!DOCTYPE html>
<html lang="en">
...
<body>
    ...
    <main class="min-h-[56vh] mb-5">
        <section class="w-[70%] mx-auto flex flex-row flex-wrap gap-5 justify-center">
            <?php if ($cat_null != array()) { ?>
                <?php foreach ($db->getSousCat(["id_cat" => $_GET['categorie']]) as $row) {
                ?>
                    <article>
                        <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                            <a href="product.php?categorie=<?= $row['Id_Categorie']; ?>" class="rounded-lg">
                                <img class="rounded-lg p-8 w-full min-h-[400px] object-cover" src="<?= $row['img']; ?>" alt="product image" />
                            </a>
                            <div class="px-5 pb-5">
                    ...
                    <article>
                        <div class="w-full max-w-sm bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                            <a href="product_info.php?product=<?= $row['Id_Produit']; ?>" class="rounded-lg">
                                <img class="rounded-lg p-8 w-full min-h-[400px] object-cover" src="<?= $row['Photo']; ?>" alt="product image" />
                            </a>
                            <div class="px-5 pb-5">
                                ...
                                <div class="flex items-center mt-2.5 mb-5 justify-between">
                                    ...
                                    <a href="#" class="">
                                        <h5 class="text-xl font-semibold tracking-tight text-gray-900 dark:text-white"><?= $row['Nom_court']; ?></h5>
                                    </a>
                                    <div class="flex">
