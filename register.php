...
if (isset($_POST['register'])) {
    $num = $db->countNumClient()[0]['count(Num_Client)'] + 1;
    $mail = htmlentities(stripcslashes($_POST['email']));
    $login = htmlentities(stripcslashes($_POST['login']));
    $prenom = htmlentities(stripcslashes($_POST['first_name']));
    $nom = htmlentities(stripcslashes($_POST['last_name']));
    $tel = htmlentities(stripcslashes($_POST['phone']));
    $societe = htmlentities(stripcslashes($_POST['company']));
    $password = htmlentities(stripcslashes($_POST['password']));
    $rpt_pass = htmlentities(stripcslashes($_POST['repeat_password']));

    $passhash = password_hash($password, PASSWORD_DEFAULT);

    if (!empty($mail) && !empty($prenom) && !empty($nom) && !empty($tel) && !empty($password) && !empty($rpt_pass) && !empty($login)) {
        if ($password == $rpt_pass) {
            $db->register(["id" => 1, "log" => $login, "pass" => $passhash]);
            $db->registerClient(["societe" => $societe, "nom" => $nom, "prenom" => $prenom, "mail" => $mail, "tel" => $tel, "commerce" => rand(1, 2), "type" => 1, "delai" => 0, "num" => "CL000000" . $num, "user" => $db->getLastId()['max(Id_User)']]);
        } else $error = "Les mots de passe ne sont pas identique";
    } else $error = "Certains champs sont vides";
}
        ...
        <section class="">
            <form class="max-w-md mx-auto" method="post">
                ...
                <div class="relative z-0 w-full mb-5 group">
                    ...
                    <label for="password" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Login</label>
                </div>
                <div class="relative z-0 w-full mb-5 group">
                ...
                <div class="grid md:grid-cols-2 md:gap-6">
                    <div class="relative z-0 w-full mb-5 group">
                        ...
                        <label for="phone" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Numéro de tél (0600000000)</label>
                    </div>
                    <div class="relative z-0 w-full mb-5 group">
                        <input type="text" name="company" id="company" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none  dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
                        <label for="company" class="peer-focus:font-medium absolute text-sm text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-600 peer-focus:dark:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Nom de votre société</label>
                    </div>
                </div>
                <button type="submit" name="register" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Submit</button>
            </form>
        </section>
    </main>
